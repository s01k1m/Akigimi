...
public class GenerateWithdrawAccountService {
    ...
    public CreateAccountServiceResponse create(long userId, Bank bank) {
        ...
        Optional<Account> account = queryAccountPort.findByUserAndAccountType(user, AccountType.WITHDRAW);
        String accountNumber = randomNumberPort.generateDigit(16);
        if(account.isEmpty()){
            // 중복된 계좌 번호가 있는지 확인
            for(int i=0; i<10; i++){
                Optional<Account> checkAccount = queryAccountPort.findByAccountNumber(accountNumber, AccountType.WITHDRAW);
            ...
            // 기존 반환
            Account existAccount = queryAccountPort.findByUserAndAccountType(user, AccountType.WITHDRAW)
