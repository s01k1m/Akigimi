...
class MarkLikeToFeedServiceTest extends SnsServiceIntegrationTestSupport {
    ...
    @DisplayName("[happy] 유저가 공개된 게시글에 좋아요 요청 시 정상적으로 좋아요를 완료한다.")
    @Test
    public void givenValidFeedAndUser_whenUserRequestLike_thenExecute() throws Exception {
        //given
        User user = commandUserDbPort.save(User.builder()
                ...
                .oauthId("abcde")
                .build());
        Feed feedToLike = prepareForLikeFeed(user);

        //when
        markLikeToFeedService.execute(user.getId(), feedToLike.getFeedId());
        //then
        Optional<Like> result = queryLikeDbPort.findByUserIdAndFeedId(user.getId(), feedToLike.getFeedId());
    ...
    private Feed prepareForLikeFeed(User user) {
        Product product = Product.builder()
                .price(500000)
                .detail("테스트 프로덕트입니다.")
                .image("https://testUrl.com")
